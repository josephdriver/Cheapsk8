	...
	if (game.alertLevel.threshold !== "never") {
		if (
			...
			(game.alertLevel.threshold === "cheapest" &&
				parseFloat(game.lowestPrice) <=
					parseFloat(game.lowestPriceEver))
		) {
			isAlert = true;
		}
	}
...
export const getAlertTime = (
	...
	switch (alertLevel.threshold) {
		...
		default:
			if (
				parseFloat(highestPercent) >=
					parseInt(alertLevel.threshold, 10) &&
				!alertTime
			) {
				return new Date().getTime();
			}
...
export const getGameReferenceValues = (deals) => {
	...
	deals.forEach((d) => {
		if (
			!highestPercent ||
			parseFloat(d.savings) > parseFloat(highestPercent)
		) {
