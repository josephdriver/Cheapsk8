...
function ContentBlock({ deals, savedStores, stores }) {
    ...
    rawData.map((item) => {
      ...
      item.data.map((dealItem, index) => {
        if (dealItem.steamAppID && largeCardCount < largeCardQty) {
          storeObj.data.push({ row: [dealItem] });
          largeCardCount += 1;
      ...
      {data.map((store) => (
        <View style={{ marginHorizontal: 10 }}>
          ...
          <View style={{ marginVertical: 10 }}>
            {store.data.map((item) =>
              item.row.length === 1 && item.row[0].steamAppID ? (
                <LargeCard key={item.row[0].dealID} deal={item.row[0]} />
              ) : (
